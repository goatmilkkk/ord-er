function calculateOrdDate() {
    ...
    if (batch.length == 0 || year.length == 0 || isNaN(batch) || isNaN(year) || parseInt(batch) < 1 || parseInt(batch) > 500 || ![3, 4].includes(parseInt(year))) {
        document.getElementById("ord-date").innerHTML = "Invalid<br>Input";
        return;
    }
    ...
    if (daysTillOrd > 0) {
        document.getElementById("ord-date").innerHTML = "You ORD in<br>" + daysTillOrd + " days";
    } else {
        document.getElementById("ord-date").innerHTML = "ORD LO!";
    }
}
